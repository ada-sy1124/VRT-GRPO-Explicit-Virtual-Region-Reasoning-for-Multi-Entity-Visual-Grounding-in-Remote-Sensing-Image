"""Run Qwen VRT inference with stacked SFT and GRPO LoRA adapters."""
...
GRPO_LORA_ADAPTER_PATH = os.environ.get("GRPO_LORA_ADAPTER_PATH", "/root/autodl-tmp/SPIN/outputs/output_qwen3/vrt_grpo")
AUTO_FIND_LATEST_GRPO_LORA = os.environ.get("AUTO_FIND_LATEST_GRPO_LORA", "1") == "1"

OUTPUT_JSONL = os.environ.get("OUTPUT_JSONL", "/root/autodl-tmp/SPIN/outputs/output_qwen3/vrt_test_grpo.jsonl")
...
def main():
    ...
    if rank == 0:
        print(f"Loading and merging SFT LoRA: {SFT_LORA_ADAPTER_PATH}")
    sft_model = PeftModel.from_pretrained(
        model,
        SFT_LORA_ADAPTER_PATH,
        is_trainable=False,
    )
    model = sft_model.merge_and_unload(safe_merge=True)

    grpo_adapter_path = (
        find_latest_lora_adapter(GRPO_LORA_ADAPTER_PATH)
        if AUTO_FIND_LATEST_GRPO_LORA
        else GRPO_LORA_ADAPTER_PATH
    )
    if rank == 0:
        print(f"Loading independent GRPO LoRA: {grpo_adapter_path}")
